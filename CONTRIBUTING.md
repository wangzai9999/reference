...
感谢您对**备忘清单**贡献的兴趣👍👍，是像您这样的人使 [`Quick Reference`](https://ref.xszhome.cn) 成为如此出色的网站 🎉🎉。随时提交问题和增强请求。
...
只需要 `标题<h1>` 和 `介绍` (标题下面)。脚本会自动识别，通过 GitHub Actions 自动发布 [`Quick Reference`](https://ref.xszhome.cn) 网站。
...
[`Quick Reference`](https://ref.xszhome.cn) 使用 [`@wcj/markdown-to-html`](https://github.com/jaywcjlove/markdown-to-html) 转换 `Markdown`，并使用 [`rehype-attr`](https://github.com/jaywcjlove/rehype-attr) 插件让其支持通过其注释语法添加类和样式。此外，您可以在 Quick Reference 备忘清单上使用样式参考：<https://ref.xszhome.cn/docs/quickreference.html>

最后，参考现有备忘清单的源代码是一个好习惯！

## 首页导航

[`Quick Reference`](https://ref.xszhome.cn) 的首页存放在仓库的根目录 `README.md`，[`Quick Reference`](https://ref.xszhome.cn) 是通过这个 `README.md` 自动生成首页导航，下面是导航实例：
